...
CMD2 = ('E2E代2考题：续接这条战线——先看懂上代战况与产物（战线档案），'
        '再基于上代清单新增 summary/e2e-summary.md：引用上代 token 与 items 数值，'
        '并写一句说明本任务如何续在上代成果上。'
...
def dump_campaign(task_id):
    ...
    if os.path.exists(src):
        shutil.copy(src, f'{EV}/r-e2e-campaign-{task_id}.jsonl')
        log('campaign dumped:', task_id)


def dump_directives():
    if os.path.exists('.smoke-state/directives.jsonl'):
        shutil.copy('.smoke-state/directives.jsonl', f'{EV}/r-e2e-directives.jsonl')
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if stage == 'issue':
            ...
            b = board_api(pg)
            with open(f'{EV}/r-e2e-board-gen1.json', 'w', encoding='utf-8') as f:
                json.dump(b, f, ensure_ascii=False, indent=1)
            if c.get('taskId'):
                dump_campaign(c['taskId'])
            dump_directives()
            # 读盘提取 T1（token 由代1 LLM 现场生成）
            manifest = os.path.join(WS, 'manifest', 'e2e-manifest.json')
            if os.path.exists(manifest):
                shutil.copy(manifest, f'{EV}/r-e2e-manifest.json')
                tok = json.load(open(manifest, encoding='utf-8')).get('token', '')
                open(f'{EV}/r-e2e-token.txt', 'w').write(tok)
                log('T1 extracted:', tok)
            else:
                log('WARN: manifest 未落盘（代1 未收官或未建文件）')
            shot(pg, 'r-e2e-02-gen1-closed.png')

        elif stage == 'issue2':
            tok = open(f'{EV}/r-e2e-token.txt').read().strip()
            assert len(tok) == 8, f'T1 缺失或形状不对: {tok!r}'
            b = board_api(pg)
            c1 = find_cmd(b, TAG1)
            assert c1 is not None, '板上找不到代1命令'
            r = post(pg, '/warroom/api/commands', {'text': CMD2, 'continuesFrom': c1['commandId'], 'name': 'e2e战线'})
            ...
            b = board_api(pg)
            with open(f'{EV}/r-e2e-board-gen2.json', 'w', encoding='utf-8') as f:
                json.dump(b, f, ensure_ascii=False, indent=1)
            c2 = find_cmd(b, TAG2)
            if c2.get('taskId'):
                dump_campaign(c2['taskId'])
            dump_directives()
            summary = os.path.join(WS, 'summary', 'e2e-summary.md')
            if os.path.exists(summary):
                shutil.copy(summary, f'{EV}/r-e2e-summary.md')
                log('summary copied')
            else:
                log('WARN: summary 未落盘')
            shot(pg, 'r-e2e-04-gen2-closed.png')

        elif stage == 'close':
            shot(pg, 'r-e2e-05-final-board.png')
            dump_directives()
            b = board_api(pg)
            with open(f'{EV}/r-e2e-board-final.json', 'w', encoding='utf-8') as f:
                json.dump(b, f, ensure_ascii=False, indent=1)
