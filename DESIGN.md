...
## V18.1 impeccable critique 三轮闭环（2026-08-29，元首令「用技能审查 UI/UX 修复到优秀」）

双子隔离协议（A=设计总监无锚定 + B=探测器/行为实证）三轮：**R1 28/40 → 整改 → R2 33/40 → 整改 → R3 34/40 确认**（B 侧 R2 修后 R3 复测 18/18 子检查 0 缺陷）。

**R1 整改（A 8 项+B 1 项）**：列头计数改切片派生（撒谎账本根除）+归档空页签安神行；live 列「常驻」声明+注释修正；管线一次性指路 toast（**不恢复 12% 常显——V17.8 舰长令优先于评审建议**）+pipe-overlay 注释矛盾修正；map-hint 移列区/调度栏沟带+dismiss 注释诚实化；归档确认键红语义（--war-fail）+gate 理由可见副行；activity-dot reduced-motion 守卫；聚焦页归档行移到决策带下；HQ 弹窗焦点锚星域容器；12px/字距/死样式/岛计数口径 title/2D 画布图例（复用 mapLegend——3D 有 2D 无的两待遇补齐）。
**R2 整改（A2 7 项）**：终局命令安神带二态实名（cancelled=✕「已取消——已终局」/settled=✓「已收官」，quiet 只留在途——读投影说假话 P1）；cancelled 生命条头段红（绿=善终契约）；归档空态单横幅（三遍同句=噪音）；岛计数**「等你 N」段居首**（收件箱同源聚合，点击钉岛亮收件箱——开屏叙事从系统状态报告变舰长待办）；composer 续接候选折叠（>3 收起，全板唯一 >4 决策簇收口）；3D safe 带让出 foot 高度（执行卡不压操作行）；链空命令不渲染永久灰归档闸。
**R3 整改（残留 3 小项）**：终局快捷操作占位二态分岔（noQuickCancelled/noQuickSettled）；岛「等你」首击 flash 推迟一拍；quiet 带 ✕ 标 err 色特例。
...
## V18.4 钉住卡冻结锚点 + 聚焦独占 + 铭文上缘外弧（2026-08-30，元首三条反馈）

**①钉住卡=点击时刻冻结屏幕锚点（舰长令）**：弃 V18.3「锚星球投影随动」——聚焦建立/切换的瞬间把锚点冻结在**当下悬停卡的位置**（光标 +18/+16，经 safe 区钳制），之后平移/缩放镜头锚点不动——「悬停卡被固定住」的体感。实现=帧环 `lastFocusWs` 变化沿捕获 `pinnedAnchor`（帧内局部变量，非 ref——随 effect 生命周期即可）。

**②聚焦态独占（舰长令）**：聚焦下非聚焦星球从悬停拾取中剔除（`hovered=null`——无悬停卡/无族高亮变化/光标保持抓取）；点击过滤在 onClickRoot 双模式（cmd/3d）——点非聚焦星球**既不换聚焦也不算空处退出**（无事发生）；HQ 点击保留（中枢非星球）。聚焦星球自身悬停照常（光标锚+战线行）。

**③铭文二稿（舰长令实测反馈「星球还很大铭文就没了」）**：根因=V18.3 门槛要求弧文塞进星球宽度（planetPx≥arcPx）——严格但不合直觉。改：**文字处于星球外、悬在星球上方**——弧排布移到上缘（canvas 圆心下移 CY=120，上弧角度 -π/2±δ **递增**方向+rotate(a+π/2)，与下弧镜像相反），弧半径压着 limb 外 4px（`offScreen=arcPx−planetPx−4`，星球更小时整体上移保持弧线在外，不要求放进星球宽度）；隐显门槛放宽到 `planetPx<8`（星球缩到接近像素点才隐）——实拍远界全景六星铭文全在。2D 雷达名签保持下缘弧不动。

**排障录（3D 悬停「失灵」两小时）**：探针报 3D 悬停全灭——逐层二分（帧环活着/RAF 4fps 节流/cmd 正常/scene.pick 手动调用命中/实例补桩显示帧环根本没调 pick）→ 真相=**探针点了 planetScreen 投影位，而该星球恰好漂到战报浮舱底下**（elementFromPoint=war-report）——浮舱拦截 pointer，星域收不到 mousemove。星球漂到舱下不可悬停是既有布局语义（V11.5f 已知），非回归；探针必须先 `elementFromPoint` 验证星球**实际暴露**再交互。教训：**3D 星球在轨道上移动，投影点位有时效性，取证前先验可点性**。

**验证**：verify PASS + shoot-v17/v7/theme 全绿 + 手测五步（聚焦显战线行/行点击开聚焦页/锚点冻结平移不变/聚焦下他星悬停点击全静默/退聚焦即收）。

## V18.5 铭文曲率动态适配（2026-08-30，元首实测圈认「长名弧与星球边缘脱锚」）

**根因**：V18.3/18.4 的弧曲率是画布常量（R=104→屏上恒 ~48px），星球屏半径随缩放连续变化——只在一个缩放档位两者重合；长名星球（如 dsh-plugin-warroom）弧宽远超星球盘面，观感「弧没锚在边缘」，放大到某档又突然对位。

**修法（曲率动态重绘）**：`refreshLabelCurvature`——弧半径 Rc = clamp((星球屏半径+4)/缩放系数, 104, 400)（弧线恒贴 limb 外 4px；下限 104 防小星球「环绕感」——放平悬在上方更合理；上限 400 防画布溢出）；缩放跨过阈值 max(3px, 8%) 才重绘（zoom 手势期间连续重绘 6-8 张小画布，代价可忽略）；逐帧定位公式独立于 Rc：弧顶画布固定位 (CX, TOP_Y=26)，sprite 中心 = 星球中心 + (0, -(Pr+4) + (H/2−TOP_Y)·scale)。canvas/ctx 挂 sprite userData 复用，needsUpdate 上传。

**验证**：verify PASS + shoot-v17/v7 全绿 + 远界全景实拍六星曲率各自适配（drawn 74~127 各异且都贴自身上缘）。取证 shots-v182/v185-*。

## V18.6 曲率下限回归 + 2D 名签上移 + 2D 战线环（2026-08-30，元首三条反馈）

**①曲率下限回归（舰长令「曲率下限挺好的，调回去」）**：V18.6 一度移除下限让长名沿球面环绕——元首复看后定案 V18.5 下限更好：Rc = clamp((星球屏半径+4)/缩放系数, **104**, 400) 回归，远界小星球铭文放平悬在上方；字号收缩逻辑（84° 扇区，下限 15）同步回归，画布 H 回 170。drawArc 通用化（半径参数化），measure 抽模块级 measureArcText（曾因嵌套 function 进 class 体炸 parse——class 内禁 function 声明）。

**②2D 名签上移（舰长令）**：2D 雷达星球名从下缘弧改**上缘外弧**（与 3D 铭文同语言：角度自 -π/2-δ 递增 + rotate(a+π/2)）；执行卡避让铭牌带随之上移（y+6→y-28）。
...
**重写收敛**：起草法 4292→3720（表格反例并入写法、对照示例压半、措辞去脂；锁定针脚「任务令起草法/大白话 → 任务书/工作区路由/【星球：】/@new:/战线跟着星球走/KillCredit」全保）；大副条令职责②的字段细则改为指向起草法（注入面互相叠加时只教一次）；外勤条令素养五条压行（出口协议段零触碰）。旧 18 份 33812→31430（-7.1%）；troop 五件入册快照门（+1784，门禁覆盖补全另计）。
...
## V20 板面收敛：读 + 下令入口（2026-09-05，舰长令）

**原则定案**：板面=读投影+下令入口（composer）；**动作集中在聚焦页的定夺点**——账面变更类按钮全部住在读得到上下文的地方，**卡面只留导航**。

**退役三件**：①命令卡 R5「进入对话」（markTalking=账面变更住在卡面）——点卡即进聚焦页，答澄清走置顶决策带（决策带本就带 markTalking+跳会话）；②命令卡 R5「改直发」（regrade 住在卡面）——改档在聚焦页配置段（能力早就在），R4 preflight 行退为纯读提示（hint/hintTalking/title 文案照旧说明后果）；③任务卡处理钮「去验收/去下重试令」直跳大副会话=动作绕开上下文——改路由聚焦页对应段（reported→report 决策带/failed→chain 决重试），与收件箱同映射，会话直达降级为聚焦页底部跳钮。copy `preflight.toDirect` 键四典（war/plain×zh/en）同删，copy-lang 键形锁两侧同删自动过。
