...
### Fixed
- **后端对抗性审查修复批（2026-09-23，4 子代理审查 → 37 项发现全数处置）**：白盒三分片+实弹探测的对抗审查后，除 1 项宿主侧问题（`/warroom` 前缀绕过宿主 token 鉴权——根因在宿主 route registry，本仓无法自查 token，需上游收口）外全部落地修复。**P0**：war_fail 重派死锁——requeue 后任务回 published，spawn-once 守卫永不释放，重派征召被静默吞掉且回执虚报「已派遣新外勤小队」、巡检也补不上（静板确定性复现）；修=requeue 落账点（war_fail+巡检 rescue 两处）同步 `releaseSpawned` + war_fail 消费征召结果、失败如实回执。**P1×4**：①已取消定时令账本无限膨胀（dueScheduledDirectives 不查 status，引信每 30s 追加被终态守卫吞掉的 dispatched，每天 +2880 行废账）→ 只认 draft；②收官判定 `includes('通过')` 子串误判（「打回：验收未通过」折成 succeeded 胜局，双路独立发现）→ `isPassVerdict` 词头+句读边界匹配（认可 通过/通过收官/合格/验收合格/验收通过）；③征召计划不滤 deps（dep-locked 任务提前征召空转+遮蔽同工作区可跑任务）→ conscriptPlan 跳过依赖未满足；④大副侧动词零身份校验（任意 agent 可 war_close_task 对 in_progress 任务「通过」收官、伪造舰长批注、越权 deploy/orders/recall/log_report——强制人工验收此前只靠提示词维持）→ 五道身份/状态门（close 通过语义须 reported+所属大副/HQ 会话，孤儿任务 legacy 宽容）。**P2×9**：HTTP 状态码从不写入（`void code` 全部错误 200 出线，实弹证）→ send() 真实落码+测试 helper 真捕获（历史假 200 掩盖面清除）；cron 空段/尾随垃圾静默放宽（'1,,2' 变每天发）+ 不可满足表达式入库后每次求值烧 0.65s → 严格解析+assertCronUsable 创建/发布双侧校验；war_publish 从不校验 cron → 落账前拒绝；KillCredit 不否决 `tests.failed>0`（exit_code 0+failed 3 照过机械全绿）→ 双处一票否决；崩断半行+重启粘连吞两条事件 → jsonl.ts 追加唯一入口（尾字节检测补 \n）+ pivot/publish 连写 appendJsonlBatch 单次 write；工作区互斥不认 junction 别名（本机 C:\Users≡d:\users 判不冲突）→ best-effort realpath 归一；L2 goal disarm 失败无补偿（armed goal 残留参谋名下）→ clear 降级；toolFilter 自愈裁剪 fail-open 静默 → loud warn；归档部分失败即终局（超时会话永无补档通道+账实分离）→ 幂等续档（缺啥补啥、事件合并）。**P3×21 全清**：relay 毒命令中断同 tick、定时任务令无停用路径（打回归闭落 enabled:false+dueBounties 过滤）、rescue 连败无退避（≥8 次隔离）、state.json 原子写、同秒 ID 碰撞（后缀 4→8 hex）、V18 物化根守卫失配、normalizeFront 大小写、SSE close 竞态泄漏+自清、readBody 64KB 上限+断线不挂起、坏 JSON→400 干净文案、反射截断 120 字符、planets 首请求 ENOENT、fold-cache 冻结防毒化、ActivityTracker LRU、user-seen 封顶、fileGuard 大小写不敏感、注释 60s→90s 等。验证：`pnpm verify` PASS（tests+build+bundle 针脚三段式，终态 365/365 测）；审查轮证据=四子代理报告（白盒三分片+实弹 12 发拒绝路径零误触发）。内核同名文件（index/tools/events/directives/rules/goals）改动建议 stardeck 侧评估回流。
...
> 版本头补切（发版门勘误）：v0.20.3 发版时 [Unreleased] 尚空（文档条目随发版后的提交落账），
> 本头按 tag v0.20.3（d6f97b5）实发内容回补。goal 原文写「0.21.0」，正典语义按 release.mjs
> minor=迭代+1（0.20.2→0.20.3）推进——偏差如实记，下版起发版前先切头。
